...
const standaloneHtml = readFileSync(join(__dirname, "dist/standalone.html"), "utf-8");
...
// Serve standalone HTML for all page requests
app.get("/", (req, reply) => {
...
app.setNotFoundHandler((req, reply) =>
  req.raw.method === "GET" && req.headers.accept?.includes("text/html")
    ? reply.type("text/html").send(standaloneHtml)
    : reply.code(404).send({ error: "Not Found" })
);

const host = process.env.HOST || "0.0.0.0";
app.listen({ port, host }).then(() => console.log(`Server running on http://${host}:${port}`));